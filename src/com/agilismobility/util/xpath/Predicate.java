 ...
 * @author Cosmin
 * 
 *         since I don't want to implement a full-blown xpath engine our
 *         predicate
 */
public class Predicate {
	...
	Predicate(Vector inNodeSet, String predicateExpr) {
		...
		if (predicateExpr == null || predicateExpr.length() <= 0) {
			...
			// we need to parse predicateExpr
			resultSet = new Vector();
		// check if this predicate is just a logical condition or a complete
		// XPath query.
		// for now we support only logical conditions
		/*
		 * If it is a complete XPath expression { ResultSet = (new
		 * XPathExpression(XML_DOM, XPath Expression)) .getResult(); }//If it is
